...
def format_volumes(vols):
    ...
    for volume in vols:
        if volume["mountpoint"] is not None:
            if (
                input("\nWARNING! Some volumes are mounted. Continue? [y/N] ").lower()
                == "y"
            ):
                break
            else:
    ...
    for volume in vols:
        if volume["mountpoint"] is None:
            match volume["name"]:
                ...
                case _:
                    if (
                        input("\nFormat /dev/" + volume["name"] + "? [y/N] ").lower()
                        == "y"
                    ):
...
def pacstrap(hostname):
    ...
    with open("/mnt/boot/loader/entries/arch.conf", "w") as f:
        ...
        f.write(
            "options root=" + uuid + " rw nowatchdog loglevel=3 nvidia_drm.modeset=1\n"
        )
    ...
    os.system(
        "arch-chroot /mnt wget https://raw.githubusercontent.com/ialebedev/shell_tools/main/setup_vfxstation.py -P /root"
    )
