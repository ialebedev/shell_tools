...
def install_soft():
    # COMMON SOFT
    os.system(
        "pacman -S --needed --noconfirm sudo man tmux rsync bash-completion exfat-utils ntfs-3g libunrar unrar zip unzip p7zip dkms duf htop eza bat yazi zoxide fzf util-linux ethtool numactl btop ncdu openresolv"
    )
    ...
    if input("Install soft for vfxstation? [y/N] ").lower() == "y":
        os.system(
            "pacman -S --needed --noconfirm sddm plasma-desktop plasma-pa plasma-systemmonitor kdeplasma-addons breeze breeze-gtk kde-gtk-config konsole thunar thunar-volman gvfs thunar-archive-plugin pipewire pipewire-pulse pipewire-alsa okular spectacle filelight filezilla firefox firefox-ublock-origin ttf-dejavu ttf-liberation ttf-bitstream-vera cantarell-fonts ark kscreen krename kate ktorrent kolourpaint mpv mediainfo inkscape python-pyqt5 python-lxml telegram-desktop nvidia-open-dkms nvidia-utils opencl-nvidia qt5-xmlpatterns hddtemp psensor obsidian doublecmd-qt6 libxml2-legacy xclip"
        )
